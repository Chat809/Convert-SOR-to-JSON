...
app.post("/upload", upload.single("file"), (req, res) => {
  ...
  fs.readFile(sorFilePath, "utf8", (err, data) => {
    ...
    fs.writeFile(outputJsonPath, JSON.stringify({ distances }, null, 2), "utf8", (err) => {
      if (err) {
        return res.status(500).json({ message: "Erro ao criar o arquivo JSON" });
      }

      res.json({ message: "Conversão bem-sucedida", outputFile: outputJsonPath });
    });
  });
});
